...
class nueva_venta extends fs_controller
{
   ...
   protected function private_core()
   {
      ...
      else if( isset($_REQUEST['tasa-venta']) )
      {
         $this->tasa_venta();
      }
      else if($this->query != '')
         ...
         if( $albaran->save() )
         {
            ...
            for($i = 0; $i <= $n; $i++)
            {
               if( isset($_POST['referencia_'.$i]) )
               {
                  ...
                  $linea->pvpunitario = (floatval($_POST['pvp_'.$i])/$conversion);
                  $linea->cantidad = floatval($_POST['cantidad_'.$i]);
                  $linea->dtopor = floatval($_POST['dto_'.$i]);
                  $linea->pvpsindto = ($linea->pvpunitario * $linea->cantidad);
                  $linea->pvptotal = (floatval($_POST['neto_'.$i])/$conversion);
            ...
            if($continuar)
            {
               ...
               if( abs(floatval(($_POST['atotal'])/$conversion) - $albaran->total) >= .02 )
               {
   ...
   private function nueva_factura_cliente()
   {
      ...
      if($continuar)
      {
         ...
         $factura->codpais = $direccion->codpais;
         $factura->codpostal = $direccion->codpostal;
         $factura->coddir = $direccion->id;
         $factura->direccion = $direccion->direccion;
         $factura->provincia = $direccion->provincia;
         ...
         else if( $factura->save() )
         {
            ...
            if($continuar)
            {
               ...
               if( abs((floatval($_POST['atotal'])/$conversion) - $factura->total) >= .02 )
               {
                  $this->new_error_msg("El total difiere entre la vista y el controlador (".($_POST['atotal']/$conversion).
   ...
   private function nuevo_presupuesto_cliente()
   {
      ...
      if($continuar)
      {
         ...
         $presupuesto->tasaconv =floatval($tasaconv);
         ...
         $presupuesto->ciudad = $direccion->ciudad;
         $presupuesto->codpais = $direccion->codpais;
         $presupuesto->codpostal = $direccion->codpostal;
         $presupuesto->coddir = $direccion->id;
         $presupuesto->direccion = $direccion->direccion;
         $presupuesto->provincia = $direccion->provincia;
         ...
         if( $presupuesto->save() )
         {
            ...
            if($continuar)
            {
               ...
               if( abs((floatval($_POST['atotal'])/$conversion) - $presupuesto->total) >= .02)
               {
                  $this->new_error_msg("El total difiere entre el controlador y la vista (".
                          $presupuesto->total." frente a ".($_POST['atotal']/$conversion)."). Debes informar del error.");
                  $presupuesto->delete();
               }
      ...
      if($continuar)
      {
         ...
         if( $pedido->save() )
         {
            ...
            if($continuar)
            {
               ...
               if( abs((floatval($_POST['atotal'])/$conversion) - $pedido->total) >= .02)
               {
                  $this->new_error_msg("El total difiere entre el controlador y la vista (".
                          $pedido->total." frente a ".($_POST['atotal']/$conversion)."). Debes informar del error.");
                  $pedido->delete();
               }
               else if( $pedido->save() )
               {
   ...
   //funcion para buscar la tasa de venta
   private function tasa_venta()
   {
      /// desactivamos la plantilla HTML
      $this->template = FALSE;
      ...
      $datos = $this->tasas->get($divisaemp, 'venta', $fecha, $coddivisa);

      if(!$datos){
         $datos = $this->tasas->ultima_tasa($divisaemp, 'venta', $coddivisa);
         if(!$datos){
            $datos = false;
         }
      }
